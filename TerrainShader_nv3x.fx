...
{
    return float4(calcPVPointTerrain(indata.wPos, indata.Normal), 0.0) * 0.5;
}
...
float4 psDx9_BM_MulDFast(VS2PS_BM_Dx9_MulDFast indata) : COLOR
{
    ...
    float4 light = (accumlights.w * vSunColor) + accumlights * 2.0;
...
float4 psDx9_BM_MulD(VS2PS_BM_Dx9_MulD indata) : COLOR
{
    ...
    float4 outColor = colormap * light * 4.0 * lerp(0.5, yplaneLowDetailmap.z, lowComponent.x) * lerp(0.5, mounten, lowComponent.z);
...
VS2PS_BM_Dx9_MulD vsDx9_BM_MulD(APP2VS_BM_Dx9 indata)
{
    ...
    float tot = dot(1.0, outdata.BlendValueAndWater.xyz);
