...
)
{
    ...
    if (shadowmapEnable)
    {
        ...
        outdata.SunLight = indata.TerrainLightmap.y * vSunColor * 2.0;
        outdata.TerrainColor = lerp(indata.TerrainColormap, 1.0, indata.Packed.w);
    }
    else
    {
        light += indata.TerrainLightmap.y * vSunColor * 2.0;
        outdata.TerrainColor = lerp(indata.TerrainColormap, 1.0, indata.Packed.w);
...
float4 PShader_ZOnly(VS2PS indata, uniform sampler2D colormap) : COLOR
{
    float4 base = tex2D(colormap, indata.Tex0.xy);
    base.a *= Transparency_x8.a * 4.0;
    base.a += base.a;
    return base;
}
