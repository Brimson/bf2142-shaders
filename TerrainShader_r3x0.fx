...
vec4 psDx9_detailDiffuse(VS2PS_vsDx9_detailDiffuse indata) : COLOR
{
        ...
        vec4 detailout = (detailmap * (1.0 - indata.BlendValueAndFade.w)) + (lowDetailmap.z * indata.BlendValueAndFade.w);
        ...
        vec4 yplaneLowDetailmap = 2.0 * tex2D(sampler3Wrap, indata.Tex2);
        vec4 xplaneLowDetailmap = 2.0 * tex2D(sampler3Wrap, indata.Tex3.xy);
        vec4 zplaneLowDetailmap = 2.0 * tex2D(sampler3Wrap, indata.Tex3.zw);
        vec3 blendValue = indata.BlendValueAndFade.xyz;

        scalar chartcontrib = dot(vComponentsel, component);

        scalar color = lerp(1.0, yplaneLowDetailmap.z, saturate(dot(lowComponent.xy, 1.0)));
        scalar totBlendValue = dot(blendValue.xyz, 1.0);
        scalar blue = dot(float3(xplaneLowDetailmap.y, yplaneLowDetailmap.x, zplaneLowDetailmap.y), blendValue.xyz / totBlendValue);
        color *= lerp(1.0, blue, lowComponent.z);

        vec4 lowDetailmap = color;
        vec4 bothDetailmap = detailmap * lowDetailmap;
        vec4 detailout = 2.0 * lerp(bothDetailmap, 0.5 * lowDetailmap, indata.BlendValueAndFade.w);
...
vec4 psDx9_detailDiffuseMounten(VS2PS_vsDx9_detailDiffuseMounten indata) : COLOR
{
        ...
        vec4 detailout = ((detailmap * (1.0 - indata.ColorAndFadeLerp.w)) + (lowDetailmap.z * indata.ColorAndFadeLerp.w));
...
vec4 psDx9_diffuseLOD1Plus(VS2PS_vsDx9_diffuseLOD1Plus indata) : COLOR
{
    #ifndef USE_PLANE_MAPPING
        vec4 colormap = tex2D(sampler0Clamp, indata.Tex0.xy);
        vec4 lowDetailmap = 2.0 * tex2D(sampler3Wrap, indata.Tex0.zw);
        return lowDetailmap.z * colormap;
    #else
        vec4 colormap = tex2D(sampler0Clamp, indata.Tex0.xy);
        vec4 lowComponent = tex2D(sampler2Clamp, indata.Tex0.xy);
        vec4 yplaneLowDetailmap = 2.0 * tex2D(sampler3Wrap, indata.Tex0.zw);
        vec4 xplaneLowDetailmap = 2.0 * tex2D(sampler3Wrap, indata.Tex2.xy);
        vec4 zplaneLowDetailmap = 2.0 * tex2D(sampler3Wrap, indata.Tex2.zw);
        vec3 blendValue = indata.BlendValue;

        scalar color = lerp(1.0, yplaneLowDetailmap.z, saturate(lowComponent.x+lowComponent.y));
        scalar totBlendValue =dot(blendValue.xyz, 1.0);
        scalar blue = dot(float3(xplaneLowDetailmap.y, yplaneLowDetailmap.x, zplaneLowDetailmap.y), blendValue.xyz / totBlendValue);
        color *= lerp(1.0, blue, lowComponent.z);
